...
void AudioPlayer::play() {
    ...
    if (SDL_PutAudioStreamData(stream_, wavBuffer_, wavLength_) < 0) {
...
void AudioPlayer::setPanning(float pan) {
    if (pan < -1.0f || pan > 1.0f) {
        std::cerr << "Panning must be between -1.0 and 1.0" << std::endl;
        return;
    }
    SDL_SetAudioStreamPanning(stream_, pan);
}